...
h [a',b',c',d',e'] = "(" ++ show (round a') ++ f b' ++  show (round c') ++ ")" ++ f d' ++ show (round e') ++ " = 20" ++ "\n"

h2 [a',b',c',d',e'] = show (round a') ++ f d' ++  "(" ++ show (round c') ++ f b' ++ show (round e')++ ") = 20" ++ "\n"

h3 [a',b',c',d',e',f',g'] = "(" ++ show (round a') ++ f b' ++ show (round c') ++ ")"  ++ f d' ++ "(" ++ show (round e') ++ f f' ++
                            show (round g') ++ ") = 20" ++ "\n"

h4 [a',b',c',d',e',f',g'] = "((" ++ show (round a') ++ f b' ++ show (round c') ++ ")" ++
    f f' ++ show (round e') ++ ")" ++ f d' ++ show (round g') ++ ") = 20" ++ "\n"

h5 [a',b',c',d',e',f',g'] = "(" ++ show (round e') ++ f f' ++ "(" ++ show (round a') ++
  f b' ++ show (round c') ++ "))" ++ f d' ++ show (round g') ++ ") = 20" ++ "\n"

h6 [a',b',c',d',e',f',g'] = show (round g') ++ f d' ++ "((" ++ show (round a') ++ f b' ++
  show (round c') ++ ")" ++ f f' ++ show (round e') ++ ") = 20" ++ "\n"

h7 [a',b',c',d',e',f',g'] = show (round g') ++ f d' ++ "(" ++ show (round e') ++ f f' ++
  "(" ++ show (round a') ++ f b' ++ show (round c') ++ ")) = 20" ++ "\n"

--main = do
    --mapM_ putStrLn (map h (calc 4 2 4 6))
    --mapM_ putStrLn (map h2 (calc2 4 2 4 6))
